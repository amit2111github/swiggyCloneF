...
import Location from './Customer/Location.js'
import { getUserCoordinates, getUser } from '../../apicaller'
...
const Navigator = () => {
  ...
  const location = getUserCoordinates()
  const [placeName, setPlaceName] = useState('')

  useEffect(() => {
    setName(user.name)
    setPlaceName(location?.place_name || 'UP, India')
  }, [])
  return (
        ...
        <div className='col-lg-6'>
          <div className='container'>
            <nav className='d-flex'>
              ...
              <div className='nav-item'>
                <Link className='nav-link'>
                  <i className='fa fa-support'>{/* empty */}</i> Help
                </Link>
              </div>
